...
browser.webRequest.onCompleted.addListener(
  async (details) => {
    try {
      ...
      if (details.originUrl && details.originUrl.includes("moz-extension")) {
        return;
      }
      if (!authToken) {
      ...
      const cookies = await new Promise((resolve, reject) =>
        browser.cookies.getAll({ domain: "chatgpt.com" }, (cookieList) => {
      ...
      if (data.items && data.items.length > 0) {
        browser.storage.local.get("conversations", (result) => {
          ...
          browser.storage.local.set({ conversations: uniqueConversations });
        });
      } else {
